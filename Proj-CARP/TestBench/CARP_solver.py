...
class Route:
    def __init__(self, tasks=None, cost=0, remain_cap=None):
        ...
        """
        self.tasks: List[Task] = deepcopy(tasks) if tasks is not None else [
            gv.task_dict[0]]
        self.cost = cost
...
class Solution:
    ...
    def gene_solu(cls):
        ...
        for _ in range(num_vehicles):
            ...
            route = Route(tasks=[dummy_task],
                          remain_cap=gv.capacity)  # 初始有一个0号
            # 下面的循环生成一条route
            while True:
                valid_tasks = []
    ...
    def __repr__(self):
        s = "solution cost:{}, {}\n".format(
            self.cost, "feasible" if self.feasible() else "infeasible")
...
def read_file(path):
    ...
    for c in contents[9: 9 + num_required_edges + num_non_required_edges]:
        ...
        if c[3] != 0:
            ...
            task_dict[id + num_required_edges] = Task(
                id + num_required_edges, n2, n1, c[2], c[3])
            total_demand += c[3]
            id += 1
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def merge_split(solu: Solution):
            ...
            while True:
                ...
                if len(valid_tasks) == 0:
                    ...
                    if len(min_deadhead_tasks) == 1:
                        ...
                        task = rule(tasks=valid_tasks,
                                    remain_cap=route.remain_cap)
    ...
    unserved_tasks = [task for task in routes1.tasks] + \
        [task for task in routes2.tasks]
    # ! 不要遗漏反方向的
    ...
    # 一共有五种，每种里有2条路线
    new_routes_list = [_ms(unserved_tasks, rule)
                       for rule in [rule1, rule2, rule3, rule4, rule5]]

    best_routes = min(
        new_routes_list, key=lambda routes: routes[0].cost + routes[1].cost)
...
def single_insert(solu: Solution):
    ...
    if insert_route_idx is not None:
        solu[insert_route_idx].insert_task(
            insert_task_idx, task.invert_task if invert_task_flag else task)
    solu.calc_cost()
    return solu
...
def double_insertion(solu: Solution):
    ...
    d_tasks = [remove_route[remove_task_idx],
               remove_route[remove_task_idx + 1]]
    d_tasks_inv = [d_tasks[1].invert_task, d_tasks[0].invert_task]

    min_cost = remove_route.remove_task(
        idx=remove_task_idx) + remove_route.remove_task(idx=remove_task_idx) + 100000
    ...
    for i in range(len(solu.routes)):
        ...
        if r.remain_cap - d_tasks[0].demand - d_tasks[1].demand >= 0:
            for j in range(1, len(r.tasks) - 1):
                cost1 = r.insert_cost(
                    j, d_tasks[0]) + r.insert_cost(j + 1, d_tasks[1])
                cost2 = r.insert_cost(
                    j, d_tasks_inv[0]) + r.insert_cost(j + 1, d_tasks_inv[1])
                if cost1 < min_cost:
                    min_cost = cost1
    ...
    if insert_route_idx is not None:
        if invert_flag:
            ...
            solu[insert_route_idx].insert_task(
                insert_task_idx + 1, d_tasks_inv[1])
        else:
...
if __name__ == "__main__":
    ...
    args_list = [(20, seed + i, timeout - 4 - preprocess_time, info)
                 for i in range(8)]
